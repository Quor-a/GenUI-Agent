...
| **本地交互器** | A2UI 式客户端自持状态（toggle/expand/counter/check/tabs），零 AI 往返 |
| **生成自检闭环** | AI 写完四道闸门（可渲染/闭合/配色/乱码）→ 不合格自动修正 → 通过才结束 |
...
- **生成后自检闭环**：Agent 输出后自动校验（JSON 可渲染/闭合/配色合法/无乱码），不合格自动打回修正，通过才交付
- **通知组件家族**（30 类型）· **Expressive 35 形状库**（纯 Path 零依赖）· **语法高亮**（11 语言）
- **GFM Markdown 增强**（表格/任务列表/Alerts，流式安全）· **截断 JSON 自动闭合引擎**
- **四锁手势抽屉** · **漂浮宠物**（JSON 换装+状态播报）· **层栈导航**（任何按钮压一层）
- **HTML 通道 CDN 外部依赖全通**
