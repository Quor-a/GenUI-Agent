...
import com.genui.aiapp.agent.AgentThoughtChain
...
data class ChatState(
    ...
    val modelConfig: GenUIModelConfigData = GenUIModelConfigData(),
    val persona: PersonaConfig = PersonaConfig.defaultGenUIExpert,
    val conversationHistory: List<GenUIChatMessage> = emptyList(),
    val hasUI: Boolean = false,
    val debugInfo: String? = null,
    val pageStack: List<String> = emptyList(),
    val works: List<com.genui.aiapp.data.GenUISessionStore.WorkItem> = emptyList(),
    /** AI 自动人格孵化的人格卡（null=未孵化） */
    val soul: com.genui.aiapp.soul.Soul? = null,
    /** 灵魂孵化中 */
    val soulIncubating: Boolean = false,
    /** 新架构通道页（Markdown / A2UI 扁平表 / 内联 HTML），非空时全屏显示 */
    val channel: com.genui.aiapp.renderx.ChannelPage? = null
) {
    /** 是否可以发送消息 */
